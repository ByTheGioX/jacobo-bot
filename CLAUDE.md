...
**16. `fave_featured` es OBLIGATORIO para salir en los listados (incidente "52 en admin / 21 en público", jun 2026).** El orden "destacadas primero" de Houzez hace INNER JOIN sobre el meta `fave_featured`: un post `publish` con taxonomía y status correctos pero SIN ese meta NO aparece en ningún listado ni archivo (sí aparece en búsqueda nativa y sitemap — eso despista al diagnosticar). El publisher actual ya lo escribe (`fave_featured=0`); los posts creados antes de ese fix quedaron invisibles. Además, cuando la BD pierde `wp_post_id` (borrado masivo del incidente 15 + restore parcial), los posts viejos quedan **huérfanos**: si alguien los re-publica en bloque (acción de admin o un publish_drafts sin blindar), el admin infla el contador con copias invisibles del mismo inmueble (mismo ID de Idealista en 2-3 posts de eras distintas). Resolución aplicada: 20 copias muertas → draft, 12 únicas corregidas (slug `en-venta`, ID `3VCO-`, `fave_featured`) y ahora visibles, 11 IDs que exponían el nombre de agencia → códigos cortos (incidente 8). Blindajes:
  - `publish_drafts` ahora SOLO publica borradores trackeados en la BD; los huérfanos se listan pero no se tocan.
  - `python -m tools.draft_stale_posts` (dry-run) — despublica posts de la era vieja (`fave_property_id` = `idealista-...`); nunca toca posts trackeados, manuales ni con código de agencia.
  - `python -m tools.adopt_orphan_posts` (dry-run; **ejecutar en el VPS**) — re-vincula la BD con posts huérfanos legítimos para que el bot NO los re-cree como duplicados en el próximo ciclo.
  - `diagnose_wp_listing` ahora chequea también `fave_featured` en `_REQUIRED_METAS`.
