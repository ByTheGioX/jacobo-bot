...
def _wa_is_connected():
    ...
    try:
        ...
        if 'data-testid="qrcode"' in src or 'data-ref=' in src:
            return False
        # Only use specific, reliable markers — 'side' and '_pn_list_' match too early
        # during WhatsApp's loading animation and cause false positives.
...
def _open_wa_chat(link):
    ...
    wb.get(clean_link)
    # 5s instead of 3s — WhatsApp navigation triggers JS that takes time to settle
    time.sleep(5)
...
def scrape_photo_group():
    ...
    try:
        ...
        # Active wait: up to 20s polling for the message list to appear
        _chat_ready = False
...
# Wait for WhatsApp UI to finish rendering before interacting with it
print("  Esperando que la UI cargue completamente...")
time.sleep(5)
